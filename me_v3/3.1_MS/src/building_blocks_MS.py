...
def init(params,verbose=True): ###
    ...
    ID = 'N%i-T%i-M%i-nu%i-mu%.1e-rf%.1e-d_in%.2f-seed%i' %(params[0],params[1],params[2],nu,params[4],params[5],params[6],params[7]) 
    ...
    return T, N, L, M, nu, mu_sim, phi_sim, E_in, ID
...
def init_pop(L,M,phi,E_in,how="oneclone"): ###
    ...
    if how=="oneclone":
        ...
        fit = np.repeat(fitness(cl[0],phi),M)
        
        return cl, ncl, fit
    
def finalise(outpath,params,stats,fits,corrs):
...
def selection(cl, ncl_temp, M, phi): ###
    
    fits = np.apply_along_axis(fitness,1,cl,phi)
