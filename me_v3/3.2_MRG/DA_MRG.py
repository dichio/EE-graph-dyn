...
simset = 'trials/rho'
ID = 'N10-T50-M4096-nu100-mu2.2e-02-rf5.0e+03-d_in1.00-d_trgt0.07-seed160318'

setpath, datapath = miner.shout_paths(ID,simset)

T, ts, N, L, M, nu, mu, phi, E_in, E_trgt = miner.get_params(datapath,verbose=True)

fs, allf, s, cs = miner.import_alltime(datapath); s = s/L

t_ex, s_ex = miner.exact_solution(T, L, nu, mu, phi, E_in, E_trgt)

miner.density_one(t_ex,s_ex,ts,s,E_trgt/L,"test")
...
miner.fit_distribs(ts,allf,myts,"test")
